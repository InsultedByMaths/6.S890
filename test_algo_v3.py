...
class Args:
    def __init__(self):
        ...
        # Batch size
        self.batch_size = 8192  # Adjust based on GPU memory capacity
...
class MFCGAlgorithm:
    ...
    def get_score_lr_scale_factor(self, step, total_steps):
        ...
        if step <= warmup_steps:
            ...
            return 1.0 + 4.0 * (step / warmup_steps)
        else:
            # Remaining 90% steps: scale LR from 10.0 down to 0.25 linearly
            remaining_steps = total_steps - warmup_steps
            factor = 10.0 + (0.1 - 10.0) * ((step - warmup_steps) / remaining_steps)
            return factor
    ...
    def compute_score_loss(self, state, score_function):
        with torch.enable_grad():
            ...
            z = torch.randn_like(score)
            fake_loss = (score * z).sum()
            divergence_est = torch.autograd.grad(fake_loss, state, create_graph=True)[0]
            divergence = (divergence_est * z).sum(dim=1)
    ...
    def train(self):
        ...
        for step in pbar:
            # Shuffle or at least chunk the batch into mini-batches
            # state shape is [batch_size, state_dim]
            indices = torch.randperm(batch_size)
            state_shuffled = state[indices]

            # Update LR for score networks
            ...
            for mb_i in range(num_mini_batches):
                ...
                state_mb = state_shuffled[start:end]

                # Compute score loss and update global score network
                global_score_loss = self.compute_score_loss(state_mb, self.global_score)
                self.global_score_optimizer.zero_grad()
                global_score_loss.backward(retain_graph=True)
                self.global_score_optimizer.step()

                # Compute score loss and update local score network
                local_score_loss = self.compute_score_loss(state_mb, self.local_score)
                self.local_score_optimizer.zero_grad()
                local_score_loss.backward(retain_graph=True)
                self.local_score_optimizer.step()

                # Sample action from actor
                mean, std = self.actor(state_mb)
                ...
                # Compute reward
                reward = self.compute_reward(state_mb, action, global_samples, local_samples)

                # Observe next state
                noise = torch.randn_like(state_mb, device=self.device) * math.sqrt(self.args.time_step)
                next_state = state_mb + action * self.args.time_step + self.args.sigma * noise

                # **Clamp the next state to [-5, 5]**
                next_state = torch.clamp(next_state, min=-5, max=5)

                # Compute TD target and error
                value = self.critic(state_mb)
                with torch.no_grad():
            ...
            # Update distributions less frequently (e.g., every few steps)
            if step % 50 == 0:
            ...
            for i, s in enumerate(next_states): 
                next_states[i] = s.detach().requires_grad_(True)

            # state = next_state.detach().requires_grad_(True)
            state = torch.concat(next_states).requires_grad_(True)
